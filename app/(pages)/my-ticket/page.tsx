...
export async function createTicket(data: {
  ...
  // Create one row per ticket, each with a unique code
  const ticketRows = Array.from({ length: data.quantity }, () => ({
    ...
    payment_screenshot: data.paymentScreenshot  null,
    status: "pending",
  }))
...
export async function markTicketAsUsed(
  ...
  [3 / 26 / 2026 10: 37 AM] Abel: if (existingTicket.status !== "confirmed") {
    return { ticket: null, error: "Ticket payment not confirmed yet" }
  }
  [3 / 26 / 2026 10: 35 AM] Abel: // Mark as used
  const { data: ticket, error } = await supabase
    .from("tickets")
